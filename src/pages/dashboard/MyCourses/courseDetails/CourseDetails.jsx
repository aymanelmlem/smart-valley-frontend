...
import { courseDetail } from '../../../../redux/courses/courses.slice';
import LoadingBetweenPage from '../../../../components/LoadingBetweenPage/LoadingBetweenPage';
import { Helmet } from 'react-helmet';
import favIcon from '../../../../assets/favicon.png';
export default function CourseDetails() {
  const { isLoading, courseDtl } = useSelector((state) => state.courses);
    ...
    <div className="max-w-screen-xl mx-auto ">
      ...
      <section className="relative bg-gray-900 rounded-lg overflow-hidden dark:bg-gray-800 ">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white p-6">
          <h1 className="text-4xl font-bold">{courseDtl.course.courseName}</h1>
          <p className="mt-4 text-lg font-light">
            {courseDtl.course.courseDescription}
          </p>
          <div className=" mt-6">
            <Link
              to={`/panel/sections/${courseDtl.course._id}`}
      ...
      <section className="mt-12 bg-white p-8 rounded-lg shadow-lg dark:bg-gray-900 dark:text-gray-100">
            ...
            <div className="flex items-center space-x-4">
              <div>
                <img
                  ...
                  src={courseDtl.course.instructor.profilePicture.secure_url}
                  className="w-16 h-16 rounded-full object-cover border-2 border-gray-200 dark:border-gray-600"
                />
              </div>
              <div>
                <p className="text-gray-700 dark:text-gray-300">Teached By</p>
          ...
          <div className="flex gap-4 flex-wrap">
            {courseDtl.course.accesibleByAnyOne?.videoUrl?.map(
              (video, index) => (
                ...
                >
                  <button
                    onClick={() => handleVideoClick(video.url.split('v=')[1])}
                    className="w-full p-4 bg-black bg-opacity-60 flex items-center justify-center"
                  >
                    {courseDtl?.course.accesibleByAnyOne?.describtion[index].describtionContent}
                    <svg
